...
# per-secret `.age` files (see modules/hermes-secrets.nix and secrets/), which
# agenix decrypts to /run/agenix/<NAME> at activation; hermes-agent reads
# them via the bundled /run/agenix/hermes.env aggregate.
#
      ...
      lib.types.submodule {
        options = {
          ...
          provider = lib.mkOption {
            type = lib.types.str;
            description = "Built-in provider name or 'custom'.";
          };
          model = lib.mkOption {
            type = lib.types.str;
