"""Smoke test: verifies the E2E Playwright test infrastructure works end-to-end.

This test:
1. Starts the hermes-nicegui test server with FakeHermes
2. Launches Playwright with system Chrome
3. Navigates to the login page
4. Logs in via JS event dispatching
5. Verifies the home page loads

If this test passes, the full infrastructure chain is working.
"""
...
async def test_smoke_infrastructure_works(tmp_path: Path) -> None:
    """End-to-end smoke test: start server → login → verify home page."""
    ...
    # Unset NiceGUI test env vars so the subprocess runs in normal mode,
    # not test mode. The pytest User fixture sets these and they'd leak
    # into the child process otherwise.
    ...
    try:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                assert "Hermes" in body_text, f"Login page missing 'Hermes' text. Body: {body_text[:200]}"
                # Either "Create account" or "Log in" should appear
                has_login_or_create = ("Log in" in body_text) or ("Create account" in body_text)
                assert has_login_or_create, f"Login page missing 'Log in' or 'Create account' text. Body: {body_text[:200]}"
                ...
                assert "/login" not in current_url, f"Still on login page after submit: {current_url}"

                # Verify home page content
                body_text = await page.inner_text("body")
                assert "Hermes" in body_text, f"Home page missing 'Hermes'. Body: {body_text[:300]}"
...
def test_chrome_binary_exists() -> None:
    """Verify system Chrome binary is available."""
    assert os.path.exists(CHROME_EXEC), f"Chrome not found at {CHROME_EXEC}"
