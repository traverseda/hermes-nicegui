...
class FakeHermesCli:
    ...
    def write_run_output(self, job_id: str, filename: str, content: str) -> Path:
        """Write a cron output markdown file for `job_id` -- the scheduler
        saves each run's output under ``cron/output/{job_id}/{timestamp}.md``
        (no run id in the name, only the save-time wall clock)."""
        path = self.hermes_home / "cron" / "output" / job_id / filename
        path.parent.mkdir(parents=True, exist_ok=True)
