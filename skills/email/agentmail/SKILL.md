...
### 2. Configure MCP Server (this deployment)
On this box the MCP server is declared in `modules/hermes-service.nix` with
the key referenced as `${env:AGENTMAIL_API_KEY}` — hermes resolves that from
`~/.hermes/.env` at startup (mcp_tool.py `_env_ref_name`), so the literal key
never lands in config.yaml / the nix store. Put the actual key in the
`hermes-env` agenix secret (`.env`):
```
AGENTMAIL_API_KEY=am_your_key_here
```
The equivalent standalone config would be:
```yaml
mcp_servers:
  agentmail:
    command: "npx"
